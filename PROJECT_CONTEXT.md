...
| **Phase 1** | Data foundation: schema, 3 ingestion modules, master pipeline, smoke tests | ✅ COMPLETE (scaffold built, 5/5 tests pass) |
| **Phase 2** | Feature store + baseline logistic regression model | ⬜ NEXT |
| **Phase 3** | Production LightGBM + PMF totals + betting engine + Streamlit dashboard | ⬜ Pending |
...
### Phase 2 plan (next)
1. Team rolling-feature pipeline (5/10/20/40/82 windows × ~15 stat families)
2. Goalie rolling features with Buhlmann credibility shrinkage
3. Schedule features (rest days, travel km, B2B, timezone shift)
4. Elo rating system (K=20, home advantage ~50 pts)
5. `game_vector` materialization (join team + goalie + schedule + market, home−away differencing)
6. Baseline logistic regression, walk-forward CV, expanding window from 2014-15
7. Report log loss / accuracy / Brier / calibration plot on holdout
   - **Success gate:** baseline log loss < 0.69 (must beat naive)
